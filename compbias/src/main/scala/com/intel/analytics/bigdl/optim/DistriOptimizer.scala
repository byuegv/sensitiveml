...
object DistriOptimizer {
  ...
  private[optim] def optimize[T: ClassTag](
    ...
    // 自定义控制信息获取
    var gradInfoSum = sc.accumulator(ArrayBuffer[Double](),"epoch gradient info")(AryBufAccumulator)
    val taskDel = state.get[Boolean]("taskDel").get
    val taskStrategy = state.get[String]("taskStrategy").get
    val taskRatio = state.get[Double]("taskRatio").get
    val taskDrop = state.get[Double]("taskDrop").get

    val epochDel = state.get[Boolean]("epochDel").get
    val epochStrategy = state.get[String]("epochStrategy").get
    val epochRatio = state.get[Double]("epochRatio").get
    val epochDrop = state.get[Double]("epochDrop").get

    val layerName = state.get[String]("layerName").get
    val gradName = state.get[String]("gradName").get
    val notIteration = state.get[Int]("notIteration").get
    val useComp = state.get[Boolean]("useComp").get
    val getGradient = state.get[Boolean]("getGradient").get
    ...
    while (!endWhen(driverState)) {
      ...
      val iterationDelNum = sc.accumulator(0,"iterationdelnum")
      val curEpoch = driverState.get[Int]("epoch").get
      val curNeval = driverState.get[Int]("neval").get
      ...
      val numFinishedModelUpdates: Int = dataRDD
        .zipPartitions(models, preservesPartitioning = true) { (data, modelIter) => {
          ...
          // compute gradients one by one
          val gradInfo = new Array[ArrayBuffer[(Int,Int,Double)]](_subModelNumber)
          val selIdx = new Array[ArrayBuffer[Int]](_subModelNumber)
          if(getGradient == true) {
            for (u <- 0 until _subModelNumber) gradInfo(u) = ArrayBuffer[(Int, Int, Double)]()
            val gradientThreads = Engine.default.invokeAndWait2((0 until _subModelNumber).map(i =>
              () => {
                val localModel = cached.localModels(i)
                localModel.training()
                val localCriterion = cached.localCriterions(i)
                val input = miniBatchBuffer(i).getInput().toTensor
                val target = miniBatchBuffer(i).getTarget().toTensor
                val bs = input.size(1)
                var oloss: Double = 0.0
                val grad = Tensor[T]()
                for (cur <- 1 to bs) {
                  val preGrad = localModel.getParametersTable()[Table](layerName)[Tensor[T]](gradName)
                  if (bs == 1) grad.resizeAs(preGrad).copy(preGrad)
                  else grad.copy(preGrad)
                  val oInput = input.narrow(1, cur, 1)
                  val oTarget = target.narrow(1, cur, 1)
                  val output = localModel.forward(oInput)
                  oloss += ev.toType[Double](localCriterion.forward(output, oTarget))
                  val errors = localCriterion.backward(output, oTarget)
                  localModel.backward(oInput, errors)
                  val trainGrad = localModel.getParametersTable()[Table](layerName)[Tensor[T]](gradName)
                  val gsum = grad.sub(trainGrad).abs().sum().asInstanceOf[Float]
                  gradInfo(i) += Tuple3(i, cur, if (gsum.isInfinite || gsum.isNaN) 0.0 else gsum)
                }
                i
              }
            ), timeout)
            ...
            // filter
            val mergedGrad = gradInfo.reduce(_ ++ _).sortWith(_._3 < _._3)

            if (curEpoch % 10 == 0) gradInfoSum += mergedGrad.map(_._3) // get gradient of this iteration
            val gradNum = mergedGrad.length
            for (u <- 0 until _subModelNumber) selIdx(u) = ArrayBuffer[Int]()

            if(taskStrategy == "meandiv10"){
              val mgMean = mergedGrad.map(_._3).reduce(_ + _) / gradNum
              for (u <- 0 until gradNum) {
                if (mergedGrad(u)._3 >= (mgMean / taskRatio)) {
                  selIdx(mergedGrad(u)._1) += mergedGrad(u)._2
                }
                else {
                  iterationDelNum += 1 // accumulator
                }
              }
            }
            else{
              var areaSum = 0.0
              for(u <- 0 until ((gradNum - gradNum * taskDrop).toInt)){
                areaSum += mergedGrad(u)._3
              }
              var curSum = 0.0
              for(u <- 0 until gradNum){
                curSum += mergedGrad(u)._3
                if(curSum >= areaSum*taskRatio){
                  selIdx(mergedGrad(u)._1) += mergedGrad(u)._2
                }
                else{
                  iterationDelNum += 1 // accumulator
          ...
          val trainingThreads = Engine.default.invokeAndWait2((0 until _subModelNumber).map(i =>
            () => {
              ...
              localModel.training()
              val localCriterion = cached.localCriterions(i)
              val inputX = miniBatchBuffer(i).getInput().toTensor
              val targetX = miniBatchBuffer(i).getTarget().toTensor

              val compOrigSize = targetX.size(2)
              val featureSize = inputX.size(2) / compOrigSize
              val (input,target) = if(taskDel == true && curNeval >= notIteration && getGradient){
                val bs = selIdx(i).length
                val selInput = Tensor(bs,inputX.size(2))
                val selTarget = Tensor(bs,targetX.size(2))
                var currentIndex = 1
                for(cur <- selIdx(i)){
                  selInput.update(currentIndex,inputX.narrow(1,cur,1))
                  selTarget.update(currentIndex,targetX.narrow(1,cur,1))
                  currentIndex += 1
                }
                if(useComp == false){
                  (selInput,selTarget)
                  ...
                  (selInput.narrow(2,featureSize+1,(compOrigSize - 1) * featureSize)
                    .reshape(Array(bs * (compOrigSize - 1),featureSize)),
                  selTarget.narrow(2,2,compOrigSize - 1)
                    .reshape(Array(bs * (compOrigSize - 1),1)))
                }
              }
              else{
                if(useComp == false){
                  ...
                  val bs = inputX.size(1)
                  (inputX.narrow(2,featureSize+1,(compOrigSize - 1) * featureSize)
                    .reshape(Array(bs * (compOrigSize - 1),featureSize)),
                    targetX.narrow(2,2,compOrigSize - 1)
                      .reshape(Array(bs * (compOrigSize - 1),1)))
                }
              }
              if(input.size(1) > 0){
      ...
      if (dropPercentage == 0.0 ||
        ...
        if (recordsProcessedThisEpoch >= numSamples) {
          ...
          if(curEpoch % 10 == 0){
            logger.info(s"${_header} epochGradinfo ${gradInfoSum.value.mkString(",")}")
            gradInfoSum =  sc.accumulator(ArrayBuffer[Double](),"epoch gradient info")(AryBufAccumulator)
          }
