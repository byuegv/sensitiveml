  ...
  override def accGradParameters(input: Tensor[T], gradOutput: Tensor[T]): Unit = {
    ...
    if (input.nDimension() == 3) {
      ...
      if (gradWeightMMInBatch == null) {
        ...
        if(gradWeightMMInBatch.size()(0) != batchSize) {
          gradWeightMMInBatch = Tensor[T]().resize(getGradWeightMMInBatchShape(batchSize))
      ...
      if(withBias && gradientBiasMT.size()(0) != batchSize) {
        gradientBiasMT.resize(Array(batchSize, nOutputPlane))
        gradientBiasMT.zero()
      }
...
//              println(s"inSpatialConvolution.scala: gradientBiasMT.size:${gradientBiasMT.size().mkString("x")} _i: ${_i} nGroup:${nGroup}")
