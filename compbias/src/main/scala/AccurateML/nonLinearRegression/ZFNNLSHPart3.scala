...
object ZFNNLSHPart3 {
  def main(args: Array[String]) {
    ...
    val conf = new SparkConf().setAppName("test nonlinear").setMaster("local[*]")
   ...
   /* val initW: Double = args(0).toDouble
    ...
    val numFeature:Int =5232
    val hiddenNodesN : Int =5
    val nnItN: Int = 100
    val testPath:String = "/home/hadoop/Desktop/AccurateML/GHG.test_/GHG.test"
    val dataPath:String = "/home/hadoop/Desktop/AccurateML/GHG.train_/GHG.train"
    val test100: Boolean = false
    val weightsPath:String = "nnweights"
    val isSparse:Boolean =false

    val itqbitN:Int = 1
    val itqitN:Int =20
    val itqratioN:Int = 100
    val minPartN : Int = 1
    val upBound:Int =30
    val splitN:Double = 2.0
    ...
    //此处获取压缩之后的数据 即 objectData

    println(s"objectData: ${objectData.getClass.getName} ${objectData.collect().size}")

    objectData.take(10).foreach(elem => {
      println(s"${elem._1.size}\t${elem._2.size}\t${elem._3.size}")
      println("secondLine")
      val temp1 = elem._1
      for(x <- temp1) println(s"labeledpoint: ${x.size}")
//      temp1.map{
//        elem => {
//          for(i <- 0 until elem.size){
//            println(s"features:\n${elem(i).features}\nlabel:\n${elem(i).label}")
//          }
//        }
//      }
