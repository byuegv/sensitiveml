...
/**
  * 从原始的二进制数据中导入MNIST数据集
  * 返回Array[LabeledPoint]
  */
object MnistData {
  /**
    * 加载image - 即是features
  ...
  def getTrainLabeledPoint(trainImagePath: String,trainLabelPath: String) = {
    val imageArray = loadImageData(trainImagePath)
    val labelArray = loadLabelData(trainLabelPath)
    combineData(imageArray,labelArray)
  }
  def getTestLabeledPoint(testImagePath: String,testLabelPath: String) = {
    val imageArray = loadImageData(testImagePath)
    val labelArray = loadLabelData(testLabelPath)
    combineData(imageArray,labelArray)
  }
}
